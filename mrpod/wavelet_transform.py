...
def find_scale_index(level, x="0", y="1"):
    """
    Gray code order is used here to generate indices for the scales of each
    decomposition level in a wavelet packet transform. Either 'a' and 'd'
    (approximation and detail) or '0' and '1' are used.  E.g., for a level=1
    decomposition, the possible combination will be 'ad' and 'da'. For level=2,
    it will be 'aa', 'ad', 'da', 'dd'. The indices are also ordered by their
    corresponding frequency bandpasses.
...
def time_shift(w_j, L, j, scale):
    """
    Time shift the wavelet coefficient so that it matches the features in the
    original signal temporally. Only works if the half length of the wavelet is
    even and if the wavelet is of the LA type (symlet).
...
class CompositeFilter:
    ...
    Composite filter generated from a given wavelet for a specific
    decomposition level and all the scales involved.
    ...
    def __init__(self, g=None, wavelet="D(8)"):
        ...
        wavelet : str, optional
            Name of  the scaling filter (g). Default is 'D(8)'. Built-in
            options are: 'Haar', 'D(4)', 'D(8)', 'D(10)', 'D(12)', 'LA(8)',
            'LA(12)', 'LA(16)'

            - 'D(8)': Daubechies standard wavelet with 8 elements.

            - 'LA(8)': Least-asymmetric variant of the Daubechies wavelet with
              8 elements.

            Other wavelets can be found in the following sources:

            - *Wavelet Methods for Time Series Analysis* by Percival and
              Walden.

            - Import from the ``pywt`` package using ``pywt.Wavelet()``. Note
              that only half of the wavelet length is indicated in the name of
              the wavelets. E.g., the equivalent of 'D(8)' and 'LA(8)' in
              ``pywt`` are 'db4' and 'sym4'.

        Notes
        -----
        The generated filters need to be normalized by :math:`2^{j/2}` for
        maximum overlap wavelet transforms. This is taken into account in
        ``WaveletTransform``.
        ...
        # calculate the wavelet (highpass) filter (h) paired to the given
        # scaling filter (g)
    ...
    def u_n(self, n):
        r"""
        Filter corresponds to the scale n of a level j, i.e. a selection of g
        or h.
    ...
    def filter_cascade(self, J):
        """
        Populate the collection of filters corresponding to all the j and n in
        a cascading scheme based on the specified target decomposition level J.

        Parameters
        ----------
        J : int
            Target decomposition level (or the maximum decomposition level). It
        ...
        for j in range(2, J + 1):
            ...
            for n in np.arange(2**j):  # all n-scales in level j
                ...
                _u = self.u_n(n)
                for _l in range(Lj):  # all coefficients of u_j at n of j
                    for k in range(self.L):
                        index = _l - 2 ** (j - 1) * k
                        if index < 0 or index >= len(_u_j):
                            u_j[_l] += 0
                        else:
                            u_j[_l] += _u[k] * _u_j[index]

                print("\rProcessing the node: " + path, end="")
                self.filter_nodes.update({path: u_j})

    def sqd_gain_fcn(self, j, n, fs=None, max_overlap=False):
        r"""
        Squared gain function of a given wavelet filter at subscale n and
        ...
        fs : None, 1d array, optional
            Frequencies to be used for caclulating the transfer function [0,
            1/2).
    ...
    def max_J(self, N):
        """
        Maximum decomposition level for the data series with the given wavelet
        filter. Although there is technically no upper limit for MODWT, it is
        still recommended to apply the same criterion as in the case of DWT.
        ...
        -------
        max_dec : int
            (Recommended) Maximum decomposition level for the data series with
            the given wavelet filter.
        """
    ...
    def save_filterbank(self, full_path_write):
        ...
        ----------
        full_path_write : obj
            Full path to the file, recommended to use
            "./filterbank_[filtertype]_[j].pkl" to name the file.
        """
...
class WaveletTransform:
    ...
    def __init__(
        ...
        j : int
            Target decomposition level. A value is expected if 'standard' mode
            is used.
        ...
        filterbank : None, dict, optional
            Precalculated filterbank via ``CompositeFilter()``. It is also
            possible to input custom filters in the following format:
            {Node: Coefficients}. Node has to follow the naming convention
            '[j][0]'.
        full_path_filterbank : None, obj, optional
            Path to the filterbank pickle file created using
            ``save_filterbank()`` in ``CompositeFilter()``.
        """
        if mode == "max-overlap":
            self.filter_fct = 1
        elif mode == "standard":
            ...
            raise ValueError(
                "Choose a valid DWT mode: max-overlap or standard."
            )
        ...
        if len(np.shape(X)) == 1:
            ...
            raise ValueError(
                "Input data needs to be converted into a 2-dimensional matrix "
                + "first."
            )
        ...
        if full_path_filterbank is not None:
            ...
            raise ValueError(
                "Provide the precomputed filter bank or directory to the"
                + "local file."
            )
        else:
    ...
    def index_W(self):
        ...
        if self.filter_fct == 1:  # for MODWT
            ...
            if self.N % 2 != 0:
                raise Warning(
                    "The sample length of X is not integer multiples of 2**j."
                    + "It will be truncated."
                )

            # the elements of X outside of integer multiples of 2**j will be
            # truncated
            N_j = self.N // 2**self.j
    ...
    def filter_matrix(self, j, n):
        """
        Convert the composite filter u_j into its matrix form based on the
        sample size of N.
        ...
        """
        path = "%d%d" % (j, n)
        # for MODWT a factor of 2**(j/2) needs to be applied to the standard
        # filterbank
        u_j = (
            np.array(self.filterbank[path]) / (2 ** (j / 2)) ** self.filter_fct
        )
        # the elements of u_j is assumed zero outside of the length of Lj. In
        # practice u_j is shorter than N, its periodized version is derived by
        # appending N-Lj zeros

        if len(u_j) > self.N:
            raise ValueError(
                "j too high for the data length. "
                + "Choose a lower j or a shorter filter"
            )
        u_j_p = np.append(u_j, np.zeros(self.N - len(u_j)))
        # the index_W helps to select the correct elements of u_j_p to form
        #  u_j_mat directly
    ...
    def wavelet_coeff_1d(self, j, scales):
        """
        Decompose the input data with MODWPT at specified j and with desired
        scales, along the axis=0 dimension of the data (i.e., N in an NxM data
        ). It is possible to combine multiple (discrete) scales to achieve
        ...
        -------
        W_j : ndarray
            Wavelet coefficients from decomposing input data. It has a
            dimension of len(scales)xNxM
    ...
    def power_spectrum_1d(self, j, scales):
        """
        Calculate the approximated power spectrum of the input data based on
        ``wavelet_coeff_1d``.
        ...
        power_spectrum : 1d array
            Power spectrum at specific scales of j. It has a size of
            len(scales).
        """
    ...
    def detail_bundle_1d(self, js, scales):
        ...
        desired scales, along the axis=0 dimension of the data (i.e., N in an
        NxM data).
        ...
        -------
        B_details : ndarray
            Reconstructed detail bundle of input data with the dimension of
            NxM.
    ...
    def wavelet_coeff_2d(self, j, scales):
        """
        Decompose the input data with MODWT at specified j and with desired
        scales, along both axes of the data dimension. Only data with a shape
        of NxN is admissible due to the precomputed filterbank.
        ...
        for i, n in enumerate(scales):
            u_j_mat = self.filter_matrix(j, n)
            # perform wavelet transform along each axis of the input data
            # sequentially.
    ...
    def power_spectrum_2d(self, j, scales):
        """
        Calculate the power spectrum of the input data based on
        ``wavelet_coeff_2d``.
    ...
    def detail_bundle_2d(self, js, scales):
        """
        Reconstruct the input data (2d) with MODWPT at specified j and with
        desired scales.
